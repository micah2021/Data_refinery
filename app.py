"""
app.py — Nigeria Health AI Data Refinery Dashboard (Cloud version)
Uses environment variable for DB path with safe fallback.
"""
...
def db_ready() -> bool:
    ...
    try:
        with sqlite3.connect(DB_PATH) as c:
            n = c.execute("SELECT COUNT(*) FROM feature_store").fetchone()[0]
            return n > 0
    except Exception:
        return False
...
if not db_ready():
    st.error(
        ...
        "Please ensure `nigeria.db` is available at the configured path. "
        f"Current path: `{DB_PATH}`"
    )
    st.info(
        "If you are running this locally, ensure you have run the full pipeline:\n\n"
...
if page == "🏠 Overview":
    ...
    with col1:
        ...
        if not zone_df.empty:
            fig = px.bar(
                ...
                color="zone", color_discrete_map=ZONE_COLOURS, text="rows",
    ...
    with col2:
        ...
        if not dis_df.empty:
            fig2 = px.pie(
                ...
                color_discrete_map=DISEASE_COLOURS, hole=0.45,
            )
    ...
    if not trend_df.empty:
        ...
        fig3 = px.line(
            ...
            color="disease_category", color_discrete_map=DISEASE_COLOURS,
            labels={"period": "Epidemiological Week",
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col2:
        row_limit = st.select_slider(
            "Rows to display",
            options=[100, 500, 1000, 5000, 10000], value=500,
        )

    with st.expander("⚙️ Filters", expanded=True):
        fcol1, fcol2, fcol3 = st.columns(3)
        zone_filter = disease_filter = year_filter = ""
        ...
        if selected_table in ["feature_store","socioeconomic","maternal_health","climate_health"]:
            ...
            years  = query(f"SELECT DISTINCT {yr_col} AS yr FROM {yr_tbl} ORDER BY 1"
                           )["yr"].dropna().astype(int).tolist()
            if len(years) > 1:
                yr_range = fcol3.select_slider("Year range", options=years,
                                               value=(min(years), max(years)))
                yf = "epi_year" if selected_table == "feature_store" else "year"
                year_filter = f"AND {yf} BETWEEN {yr_range[0]} AND {yr_range[1]}"
            elif years:
                yf = "epi_year" if selected_table == "feature_store" else "year"
                year_filter = f"AND {yf} = {years[0]}"

    QUERIES = {
        "feature_store": f"""
            SELECT fs.*, l.lga_name, l.state FROM feature_store fs
            JOIN lga l ON l.lga_id = fs.lga_id
            WHERE 1=1 {zone_filter} {disease_filter} {year_filter}
            ORDER BY fs.epi_year DESC, fs.epi_week DESC LIMIT {row_limit}""",
        "disease_record": f"""
            SELECT dr.*, l.lga_name, l.state, l.zone FROM disease_record dr
            JOIN lga l ON l.lga_id = dr.lga_id
            WHERE 1=1 {zone_filter} {disease_filter}
            ORDER BY dr.report_date DESC LIMIT {row_limit}""",
        "maternal_health": f"""
            SELECT mh.*, l.lga_name, l.state, l.zone FROM maternal_health mh
            JOIN lga l ON l.lga_id = mh.lga_id
            WHERE 1=1 {year_filter}
            ORDER BY mh.year DESC, mh.quarter DESC LIMIT {row_limit}""",
        "socioeconomic": f"""
            SELECT se.*, l.lga_name, l.state, l.zone FROM socioeconomic se
            JOIN lga l ON l.lga_id = se.lga_id
            WHERE 1=1 {year_filter} ORDER BY se.year DESC LIMIT {row_limit}""",
        "climate_health": f"""
            SELECT ch.*, l.lga_name, l.state, l.zone FROM climate_health ch
            JOIN lga l ON l.lga_id = ch.lga_id
            WHERE 1=1 {year_filter}
            ORDER BY ch.year DESC, ch.month DESC LIMIT {row_limit}""",
        "surveillance_alert": f"""
            SELECT sa.*, l.lga_name, l.state, l.zone FROM surveillance_alert sa
            JOIN lga l ON l.lga_id = sa.lga_id
            WHERE 1=1 {disease_filter} ORDER BY sa.alert_date DESC LIMIT {row_limit}""",
        "lga": f"""
            SELECT * FROM lga WHERE 1=1 {zone_filter}
            ORDER BY state, lga_name LIMIT {row_limit}""",
        "data_quality_log": f"""
            SELECT dql.*, l.lga_name, l.state, l.zone FROM data_quality_log dql
            LEFT JOIN lga l ON l.lga_id = dql.lga_id
            ORDER BY dql.check_date DESC LIMIT {row_limit}""",
    }

    df = query(QUERIES.get(selected_table,
               f"SELECT * FROM {selected_table} LIMIT {row_limit}"))
    ...
    with dc1:
        st.download_button("⬇️ CSV", df.to_csv(index=False),
                           f"{selected_table}_export.csv", "text/csv")
    with dc2:
        st.download_button("⬇️ JSON",
                           df.to_json(orient="records", indent=2),
                           f"{selected_table}_export.json", "application/json")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    sel_year    = col2.selectbox("Year", years,
    ...
    if agg_by == "Zone":
        map_df = query("""
            ...
            WHERE fs.disease_category = ? AND fs.epi_year = ?
            GROUP BY fs.zone ORDER BY mean_incidence DESC
        ...
        map_df = query("""
            ...
            JOIN lga l ON l.lga_id = fs.lga_id
            WHERE fs.disease_category = ? AND fs.epi_year = ?
            GROUP BY l.state ORDER BY mean_incidence DESC
        """, (sel_disease, sel_year))
        if not map_df.empty:
            fig = px.bar(
                map_df.head(20), x="state", y="mean_incidence",
                color="mean_incidence", color_continuous_scale="Reds",
    ...
    seasonal_df = query("""
        SELECT season, ROUND(AVG(incidence_rate), 4) AS mean_incidence,
               COUNT(*) AS n
        FROM feature_store
        WHERE disease_category = ? AND season IS NOT NULL
        GROUP BY season ORDER BY mean_incidence DESC
    """, (sel_disease,))
    ...
    st.subheader("Rainfall vs incidence")
    clim_df = query("""
        ...
        WHERE disease_category = ?
          AND rainfall_mm IS NOT NULL AND incidence_rate IS NOT NULL
        LIMIT 3000
    """, (sel_disease,))
    if not clim_df.empty:
        fig_c = px.scatter(
            ...
            labels={"rainfall_mm":"Monthly Rainfall (mm)",
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    years = query("SELECT DISTINCT year FROM maternal_health ORDER BY year")["year"].tolist()
    zones = query("SELECT DISTINCT zone FROM lga ORDER BY zone")["zone"].tolist()
    ...
    if mh_df.empty:
        ...
        with col2:
            ...
            fig2 = px.scatter(
                ...
                labels={"anc_coverage_pct":"ANC Coverage (%)",
        ...
        top_risk = mh_df.nlargest(10,"maternal_mortality_ratio")[[
            "lga_name","state","zone","maternal_mortality_ratio",
            "anc_coverage_pct","skilled_birth_pct","stunting_rate_pct",
            "data_quality_score",
        ]]
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        rw_df = query("""
            ...
            FROM feature_store WHERE reporting_weight IS NOT NULL
            GROUP BY zone ORDER BY mean_weight
        """)
        if not rw_df.empty:
            fig = px.bar(
                rw_df, x="zone", y="mean_weight",
                color="zone", color_discrete_map=ZONE_COLOURS, text="mean_weight",
    ...
    with col2:
        ...
        lga_type_df = query("""
            ...
            FROM feature_store WHERE lga_type IS NOT NULL GROUP BY lga_type
        """)
        if not lga_type_df.empty:
            fig2 = px.bar(
                lga_type_df, x="lga_type",
                y=["mean_weight","mean_completeness"], barmode="group",
    ...
    src_df = query("""
        ...
        FROM disease_record GROUP BY source ORDER BY records DESC
    """)
    sc1, sc2 = st.columns(2)
    with sc1:
        if not src_df.empty:
            fig3 = px.pie(src_df, names="source", values="records", hole=0.4,
    ...
    threats = [
        ("Reporting bias",      "Urban LGAs over-represented in disease records",
         "reporting_weight < 0.6 in rural zones",
         "⚠️ Mitigated via sample weighting"),
        ("Selection bias",      "Hospital records skew toward higher-SES patients",
        ...
        ("Temporal confounding","COVID-19 disrupted health-seeking behaviour (2020–2022)",
         "Post-2020 data requires careful interpretation",
         "⚠️ TimeSeriesSplit CV prevents data leakage"),
        ("Ecological fallacy",  "LGA-level patterns do not imply individual-level risk",
         "All predictions are population-level only",
         "✅ Documented in all model outputs"),
        ("External validity",   "Zone-specific patterns may not generalise across zones",
         "Zone stratification applied in feature store",
         "✅ All 6 zones represented in training data"),
        ("Rural data gap",      "Rural LGAs have lower reporting completeness",
    ...
    st.dataframe(
        pd.DataFrame(threats, columns=["Threat","Description","Signal","Mitigation"]),
        use_container_width=True, hide_index=True,
    )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    top_lga_df = query("""
        ...
        FROM feature_store fs
        JOIN lga l ON l.lga_id = fs.lga_id
        WHERE fs.disease_category = ? AND fs.incidence_rate IS NOT NULL
        GROUP BY fs.lga_id ORDER BY mean_incidence DESC LIMIT 20
    """, (sel_disease,))
    ...
    if not top_lga_df.empty:
        ...
        with col2:
            ...
            st.dataframe(
                top_lga_df[["lga_name","state","zone",
    ...
    pov_df = query("""
        ...
        JOIN socioeconomic se
            ON se.lga_id = fs.lga_id AND se.year = fs.epi_year
    ...
    if not pov_df.empty:
        fig_p = px.scatter(
            ...
            color="zone", color_discrete_map=ZONE_COLOURS, opacity=0.65,
            labels={"poverty_pct":"Poverty Headcount (%)","mean_incidence":"Mean Incidence / 10k"},
        ...
        st.info(
            "Socioeconomic data not yet joined for this selection. "
            "Ensure fix_socioeconomic.py has been run."
        )
    ...
    readiness_df = query("""
        ...
        LEFT JOIN socioeconomic se
            ON se.lga_id = fs.lga_id AND se.year = fs.epi_year
        GROUP BY fs.disease_category ORDER BY avg_quality DESC
    """)
    ...
    if not readiness_df.empty:
        ...
        fig_r = px.bar(
            ...
            color="status",
            color_discrete_map={"🟢 Ready":"#4CAF50",
                                "🟡 Partial":"#FF9800",
                                "🔴 Needs data":"#F44336"},
            text="overall_score",
        ...
        fig_r.update_layout(height=380, xaxis_tickangle=30,
                            plot_bgcolor="rgba(0,0,0,0)",
                            paper_bgcolor="rgba(0,0,0,0)",
