...
# -- SHAP compatibility bridge ---------------------------------------------
# Added so shap_explainer.py can import standard interface names
...
def load_features(disease, zone=None):
    zone_clause = f"AND fs.zone = '{zone}'" if zone else ""
    sql = f'''
...
def build_dataset(df):
    ...
    df['incidence_rate_4w_ahead'] = df.groupby('lga_id')['incidence_rate'].shift(-4)
    ...
    df = df.dropna(subset=['incidence_rate_4w_ahead'])
    feat_cols = get_feature_columns(df)
    df[feat_cols] = df[feat_cols].fillna(df[feat_cols].median())
    X = df[feat_cols]
    y = df['incidence_rate_4w_ahead']
    w = df['reporting_weight'].fillna(0.5)
    return X, y, w

class OutbreakPredictor:
    def __init__(self, disease, zone=None, **kw):
    ...
    def train(self, X, y, w, **kw):
        self.feature_cols = list(X.columns)
        result = train(disease=self.disease, zone=self.zone)
        self.cv_scores = result.get('cv_scores', {})
        return self.cv_scores
    def predict(self, X):
        return predict_outbreak(X, self.disease, self.zone)
    def predict_with_intervals(self, X):
        import numpy as np
        preds = self.predict(X)
        return pd.DataFrame({'mean': preds,
                             'lower': preds*0.85,
                             'upper': preds*1.15})
    @classmethod
    def load(cls, disease, zone=None):
        import joblib, json
        from pathlib import Path
        tag = f'{disease}_{zone or "ALL"}'
        p = cls(disease=disease, zone=zone)
        model_path = MODELS_DIR / f'outbreak_{tag}.joblib'
        meta_path  = MODELS_DIR / f'outbreak_{tag}_meta.json'
        if model_path.exists():
            p.model = joblib.load(model_path)
        if meta_path.exists():
            meta = json.loads(meta_path.read_text())
            p.feature_cols = meta.get('feature_cols', [])
            p.trained_at   = meta.get('trained_at', '')
            p.cv_scores    = meta.get('cv_scores', {})
            p.feature_importance_ = pd.DataFrame(
    ...
    def save(self):
        import joblib, json
        tag = f'{self.disease}_{self.zone or "ALL"}'
        joblib.dump(self.model, MODELS_DIR / f'outbreak_{tag}.joblib')
